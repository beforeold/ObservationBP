...
extension Environment {
  /// Creates an environment property to read a Observable object from the environment.
  ///
  /// A backport of SwiftUI's `Environment.init` that takes an observable object.
  ///
  /// - Parameter objectType: The type of the `Observable` object to read from the environment.
  @_disfavoredOverload
  public init(_ objectType: Value.Type) where Value: AnyObject & Observable {
    self.init(\.[unwrap: \Value.self])
  }

  /// Creates an environment property to read a Observable object from the environment, returning
...
@available(visionOS, unavailable)
extension View {
  /// Places a Observable object in the view’s environment.
  ///
...
private struct ObservableKey<T: Observable>: EnvironmentKey {
  static var defaultValue: T? { nil }
}

extension EnvironmentValues {
  fileprivate subscript<T: Observable>(_: KeyPath<T, T>) -> T? {
    get { self[ObservableKey<T>.self] }
    set { self[ObservableKey<T>.self] = newValue }
  }
  ...
  fileprivate subscript<T: Observable>(unwrap _: KeyPath<T, T>) -> T {
    get {
      guard let object = self[\T.self] else {
        fatalError(
          """
          No Observable object of type \(T.self) found. A View.environment(_:) for \(T.self) may \
